...
def main(argv=None):
   ...
   try:
       try:
          opts, args = getopt.getopt(argv[1:],'hvt:j:c:o:u:e:f:d:p:',['help', 'verbose', 'tsv=','json=','cols=','object=', 'unique=', 'encoding=', 'genFile=', 'dirPath=', 'headerPath='])
	  ...
	  elif option in ('-f', '--genFile'):
             genFile = value
	  elif option in ('-d', '--dirPath'):
             tsvDirPath = value
	  elif option in ('-p', '--headerPath'):
             headerPath = value

       #print (genFile,tsvDirPath,headerPath)
       
       totalCount = 0
       count = 0
       dirList=os.listdir(tsvDirPath)
       print ("stsrt",datetime.datetime.now().time())
       dict = {'dummy': 'dummy'};
       for fname in dirList:
	       ...
	       if checkFilePath(tsvFilePath):
			...
			#print line
                      string = str(line)
	              try:
                          name,add,num,q,w,e,r,t,y,u,i,o,a,s,d,f,g,h,j,n = line
			  dict[j+add+num] = 'dummmy'
		      except ValueError:
			  print "invalid line"
			#print (name,add,num)
		    ...
		    print ("total count of ",tsvFilePath," is ",count)
                    print ("total unique files till now = ",len(dict)," total file count = ",totalCount)
